module HaskellOpenGLFunctionsTH where
import GLFunction 
...
make_haskell_interface' name fs = return $ make_haskell_interface (mkName name) fs
   
make_haskell_interface :: Name -> [GLFunction] -> [Dec]
make_haskell_interface name fs = (concatMap f_to_h fs) ++ [make_gl_parent_command fs,
    make_to_resource_command_f name fs]
...
make_to_resource_command_f :: Name -> [GLFunction] -> Dec
make_to_resource_command_f name gl_functions = result where
    result  = FunD name clauses
    clauses = map make_cmd_to_r_cmd_clause gl_functions
              
make_cmd_to_r_cmd_clause f@(GLFunction f_name return_value params) = result where
    result    = Clause [pattern] body []
    pattern   = AsP (mkName "c") $ ConP (mkName ("G" ++ name)) $ [WildP]
    body      = NormalB $ foldl1 AppE [ConE (mkName "Run"), ListE fixups, VarE $ mkName "c"]
